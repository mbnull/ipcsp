...
settings = {
    ...
    "C": {"test": True, "multiple": 3, "group": 195, "top": 1, "grid": 6}
}
...
def process_results(lib, results, ions_count, test_name, printing=False):
    ...
    for idx, cryst in enumerate(results):
        if len(cryst.arrays["positions"]) == N_atoms:
            ...
            print(
                "GULP received a bad solution. Gurobi's implementation of pooling occasionally provides solutions "
    ...
    with open(os.path.join("..", "results", test_name, "energies.txt"), "w+") as f:
        for i in range(len(results)):
            if final[i] != 0:
                ...
                ase.io.write(
                    os.path.join(
                        "..", "results", test_name, f"solution{count}_lattice.vasp"
                    ),
                    results_ip[i],
                )
                ase.io.write(
                    os.path.join(
                        "..", "results", test_name, f"solution{count}_minimised.vasp"
                    ),
                    results[i],
                )
                count += 1
...
def get_cif_energies(filename, library, format="cif"):
    ...
    # Path hacks again
    cryst = ase.io.read(
        os.path.join(".", filedir / filename), format=format, parallel=False
    )
...
def benchmark():
    ...
    shutil.rmtree(os.path.join(".", "selfresult"), ignore_errors=True)
    """
    ...
    df_summary = pd.DataFrame(
        columns=["name", "grid", "group", "best_E", "expected_E", "time"]
    )
    ...
    for i in range(1, 6):
        if settings[f"SrTiO3_{i}"]["test"]:
            ...
            allocation = Allocate(
                ions_count,
                grid_size=settings[f"SrTiO3_{i}"]["grid"],
                cell_size=3.9 * multiple,
                phase=SrTiO,
            )
            ...
            energy = get_cif_energies(
                filename="SrTiO3.cif", library=SrTiO.filedir / "SrTiO/buck.lib"
            )
            if multiple > 1:
            ...
            with open(os.path.join("..", "results", "summary.txt"), "w+") as f:
                print(
                    "Non-heuristic optimisation using Gurobi with subsequent local minimisation (test equivalent to Table 1 of the paper):",
                    file=f,
                )
                print(
                    tabulate(
